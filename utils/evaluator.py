...
from utils.decorators import typeassert
from utils.cython import eval_score_matrix
from utils.cython import float_type, is_ndarray

metric_dict = {"Precision": 1, "Recall": 2, "MAP": 3, "NDCG": 4, "MRR": 5, "Coverage": 6, "Entropy": 7}
...
class Evaluator(object):
    ...
    * **Second**, this class and its evaluation metrics can automatically fit
    ...
    @typeassert(user_train_dict=(dict, None), user_test_dict=dict, item_cate_dict=dict)
    def __init__(self, dataset, user_train_dict, user_test_dict, item_cate_dict, metric, top_k, batch_size, num_thread):
        """
        ...
        if metric is None:
            metric = ["Precision", "Recall", "MAP", "NDCG", "MRR", "Coverage", "Entropy"]
        ...
        self.item_cate_dict = dict()
        self.dataset = dataset
        self.set_train_data(user_train_dict)
        self.set_test_data(user_test_dict)
        self.set_cate_dict(item_cate_dict)
        self.cate = np.array(list(item_cate_dict.values()))
    ...
    @typeassert(item_cate_dict=dict)
    def set_cate_dict(self, item_cate_dict):
        if len(item_cate_dict) == 0:
            raise ValueError("'item_cate_dict' can be empty.")
    ...
    def metrics_info(self):
        ...
        """
        metrics_show = ['\t'.join([("%s@" % re_metric_dict[metric] + str(k)).ljust(12) for k in self.top_show]) for metric in self.metrics]
    ...
    def evaluate(self, model, test_users=None):
        ...
        batch_result = []
        coverage_sum, entropy_sum, gini_sum, hit_ratio_sum, test_sum = 0, 0, 0, 0, 0
        ...
        for batch_users in test_users:
            ...
            for idx, user in enumerate(batch_users):
                if user in self.user_pos_train and len(self.user_pos_train[user]) > 0:
                    ...
                    single_test_items = self.user_pos_test[user]
                    ...
                    hit_ratio_sum += np.isin(topid, single_test_items).sum() 
                    test_sum += len(single_test_items)
        ...
        return final_result, buf, coverage_sum, entropy_sum, gini_sum, hit_ratio_sum / test_sum
